...
func instance_to_world():
	for i in range(len(Autoload.player_party)):
		...
		if inst is Character:
			...
			inst.camera_pos.x -= i + 1
			inst.stats.generate_id(i + 1)
...
func player_turn():
	...
	battle_ui.show()
	_look_at_enemy(instance_enemies[current_enemy].global_position)
...
func remove_from_turn_list(removed_object:int):
	for object in range(len(turnList) - 1):
		if turnList[object].id == removed_object:
			turnList.pop_at(object)
	for i in len(instance_enemies):
		if instance_enemies[i - 1].stats.id == removed_object:
...
func _look_at_enemy(look_pos:Vector3):
	var tween = create_tween()
	print(instance_enemies[current_enemy].global_position)
	print(camera_3d.global_position)
	var val = camera_3d.global_position.direction_to(look_pos)
	print(val)
	tween.tween_property(camera_3d, "rotation", val, 0.2)
